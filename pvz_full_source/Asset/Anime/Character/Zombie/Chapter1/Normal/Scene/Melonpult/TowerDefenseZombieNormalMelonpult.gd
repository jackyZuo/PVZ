...
func Purify() -> void :
    ...
    if Global.isMultiplayerMode and !MultiPlayerManager.isHost:
        Destroy()
        return
    var packetConfig: TowerDefensePacketConfig = TowerDefenseManager.GetPacketConfig("PlantMelonpult")
    if cell.CanPacketPlant(packetConfig):
        ...
        if Global.isMultiplayerMode and MultiPlayerManager.isHost:
            ...
            if is_instance_valid(control):
                ...
                MultiPlayerManager.SendSpawnCharacterAt("PlantMelonpult", gridPos.x, gridPos.y, _sync_id)
    Destroy()
