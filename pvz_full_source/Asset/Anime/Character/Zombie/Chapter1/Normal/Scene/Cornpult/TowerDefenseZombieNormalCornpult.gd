...
func Purify() -> void :
    ...
    if Global.isMultiplayerMode and !MultiPlayerManager.isHost:
        Destroy()
        return
    var packetConfig: TowerDefensePacketConfig = TowerDefenseManager.GetPacketConfig("PlantCornpult")
    if cell.CanPacketPlant(packetConfig):
        ...
        if Global.isMultiplayerMode and MultiPlayerManager.isHost:
            ...
            if is_instance_valid(control):
                ...
                MultiPlayerManager.SendSpawnCharacterAt("PlantCornpult", gridPos.x, gridPos.y, _sync_id)
    Destroy()
...
func ConfirmProjectile(projectileId: int, _projectileName: String) -> void :
    match _projectileName:
        "Butter":
            sprite.head.SetFliters(["Cornpult_butter"], false)
            sprite.head.SetFliters(["Cornpult_kernal"], true)
        "ButterCatapult":
            sprite.head.SetFliters(["Cornpult_butter"], true)
            sprite.head.SetFliters(["Cornpult_kernal"], false)
