...
func Purify() -> void :
    ...
    if Global.isMultiplayerMode and !MultiPlayerManager.isHost:
        Destroy()
        return
    var packetConfig: TowerDefensePacketConfig = TowerDefenseManager.GetPacketConfig("PlantFirenut")
    if cell.CanPacketPlant(packetConfig):
        ...
        if Global.isMultiplayerMode and MultiPlayerManager.isHost:
            ...
            if is_instance_valid(control):
                ...
                MultiPlayerManager.SendSpawnCharacterAt("PlantFirenut", gridPos.x, gridPos.y, _sync_id)
    Destroy()
