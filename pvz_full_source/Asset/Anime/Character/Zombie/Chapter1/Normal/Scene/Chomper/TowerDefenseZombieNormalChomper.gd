...
@export var chewTime: float = 10.0:
    set(_chewTime):
        chewTime = _chewTime
        if !is_node_ready():
            await ready
...
func _ready() -> void :
    ...
    if Engine.is_editor_hint():
        return
    checkShape.shape.b.x = - TowerDefenseManager.GetMapGridSize().x
...
func Purify() -> void :
    ...
    if Global.isMultiplayerMode and !MultiPlayerManager.isHost:
        Destroy()
        return
    var packetConfig: TowerDefensePacketConfig = TowerDefenseManager.GetPacketConfig("PlantChomper")
    if cell.CanPacketPlant(packetConfig):
        ...
        if Global.isMultiplayerMode and MultiPlayerManager.isHost:
            ...
            if is_instance_valid(control):
                ...
                MultiPlayerManager.SendSpawnCharacterAt("PlantChomper", gridPos.x, gridPos.y, _sync_id)
    Destroy()

func SwallowOver() -> void :
    Walk()
