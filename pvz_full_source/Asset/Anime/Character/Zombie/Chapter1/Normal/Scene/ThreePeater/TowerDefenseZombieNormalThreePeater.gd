...
func Purify() -> void :
    ...
    if Global.isMultiplayerMode and !MultiPlayerManager.isHost:
        Destroy()
        return
    var packetConfig: TowerDefensePacketConfig = TowerDefenseManager.GetPacketConfig("PlantThreePeater")
    if cell.CanPacketPlant(packetConfig):
        ...
        if Global.isMultiplayerMode and MultiPlayerManager.isHost:
            ...
            if is_instance_valid(control):
                ...
                MultiPlayerManager.SendSpawnCharacterAt("PlantThreePeater", gridPos.x, gridPos.y, _sync_id)
    Destroy()
