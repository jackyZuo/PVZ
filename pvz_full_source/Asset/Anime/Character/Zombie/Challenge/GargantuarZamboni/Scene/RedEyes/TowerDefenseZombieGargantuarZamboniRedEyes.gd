...
func DestroySet() -> void :
    ...
    if Global.isMultiplayerMode and !MultiPlayerManager.isHost:
        return
    var packetConfig: TowerDefensePacketConfig = TowerDefenseManager.GetPacketConfig("ZombieGargantuarRedEyes")
    ...
    if Global.isMultiplayerMode and MultiPlayerManager.isHost:
        ...
        if is_instance_valid(control):
            ...
            MultiPlayerManager.SendSpawnCharacterAt("ZombieGargantuarRedEyes", gridPos.x, gridPos.y, _sync_id, _hitpointScale, _scale.x, instance.hypnoses, 0.0, true, spawn_pos.x, spawn_pos.y, true, 40.0)
