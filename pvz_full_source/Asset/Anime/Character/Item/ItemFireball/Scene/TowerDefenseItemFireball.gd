...
func _ready() -> void :
    ...
    if Engine.is_editor_hint():
        return
    BattleEventBus.coldEffectEmit.connect(ColdEffect)
...
func DestroySet() -> void :
    var effect: TowerDefenseEffectSpriteOnce
    for i in 6:
        effect = TowerDefenseManager.CreateEffectSpriteOnce(FIRE, gridPos, "Done")
