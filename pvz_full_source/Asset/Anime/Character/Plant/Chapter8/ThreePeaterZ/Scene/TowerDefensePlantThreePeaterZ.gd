        ...
        "Spawn":
            if !sprite.pause:
                if Global.isMultiplayerMode and !MultiPlayerManager.isHost:
                    ...
                    return
                var zombie = CreateCharacter("ZombieNormalThreePeater", global_position, gridPos, 0.0)
                zombie.Rise(2.5)
                ...
                if Global.isMultiplayerMode and MultiPlayerManager.isHost:
                    ...
                    if is_instance_valid(control):
                        ...
                        MultiPlayerManager.SendSpawnCharacterAt("ZombieNormalThreePeater", gridPos.x, gridPos.y, _sync_id, instance.hitpointScale, transformPoint.scale.x, !instance.hypnoses, 2.5, true, global_position.x, global_position.y)
            timerComponent.Run("Spawn", 25.0)
