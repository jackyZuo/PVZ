...
func Run() -> void :
    ...
    run = true
    TowerDefenseManager.pausePacket = true
    TowerDefenseManager.pauseZombie = true
    await get_tree().create_timer(8.0, false).timeout
    Destroy()

func DestroySet() -> void :
    super.DestroySet()
    remove_from_group("ApplePause")
    await get_tree().physics_frame
    if get_tree().get_node_count_in_group("ApplePause") <= 0:
