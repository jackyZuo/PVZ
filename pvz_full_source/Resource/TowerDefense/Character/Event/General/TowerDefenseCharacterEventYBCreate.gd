class_name TowerDefenseCharacterEventYBCreate extends TowerDefenseCharacterEventBase
...
static func Run(target: TowerDefenseCharacter, _num = 10.0, _dieCreate: bool = false, _collect: bool = false) -> void :
    if _dieCreate:
        if !target.IsDie():
            return
    target.YBCreate(target.global_position, _num, Vector2(randf_range(-50.0, 50.0), -400.0), 980.0, _collect)
