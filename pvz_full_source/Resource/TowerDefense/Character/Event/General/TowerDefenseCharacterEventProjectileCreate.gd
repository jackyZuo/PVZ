...
func Execute(pos: Vector2, target: TowerDefenseCharacter) -> void :
    Run(target, projectileData, createNum, createGridToRange, createSpeedToRange)
...
func ExecuteProject(projectile: TowerDefenseProjectile, target: TowerDefenseCharacter) -> void :
    Run(target, projectileData, createNum, createGridToRange, createSpeedToRange, projectile.collisionFlags, projectile.camp)
...
static func Run(target: TowerDefenseCharacter, _projectileData: TowerDefenseProjectileCreateData, _createNum: int = 8, _createGridToRange: Vector4i = Vector4i(-1, -1, 1, 1), _createToRange: Vector2 = Vector2(-100, 100), _collisionFlags: int = -1, _camp: TowerDefenseEnum.CHARACTER_CAMP = TowerDefenseEnum.CHARACTER_CAMP.ALL) -> void :
    for i in _createNum:
