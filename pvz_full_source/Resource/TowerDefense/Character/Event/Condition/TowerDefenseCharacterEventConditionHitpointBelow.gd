class_name TowerDefenseCharacterEventConditionHitpointBelow extends TowerDefenseCharacterEventBase
...
static func Run(_pos: Vector2, _target: TowerDefenseCharacter, _eventList: Array[TowerDefenseCharacterEventBase], _percentage: float) -> void :
    if _target.GetCurrentHitPoint() / _target.GetTotalHitPoint() > _percentage:
        return
