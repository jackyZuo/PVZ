...
func _ready() -> void :
    ...
    if is_instance_valid(cell) && cell.IsWater():
        ...
        var _material: ShaderMaterial = backSprite.material as ShaderMaterial
        _material.set_shader_parameter("discardDownPos", (vt * (spriteGroup.global_position + Vector2(0, 100))).y)
        SetSpriteGroupShaderParameter("discardDownPos", (vt * (spriteGroup.global_position + Vector2(0, 100))).y)
        ...
        waterTween.tween_property(sprite.material, "shader_parameter/discardDownPos", (vt * (spriteGroup.global_position + Vector2(0, 48))).y, 1.0)
        waterTween.tween_property(backSprite.material, "shader_parameter/discardDownPos", (vt * (spriteGroup.global_position + Vector2(0, 48))).y, 1.0)
